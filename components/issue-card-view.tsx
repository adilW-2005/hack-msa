...
import type { Card, IssueCardPayload, User } from "@/lib/types";
...
export function IssueCardView({
  ...
  const maskedNumber = createdCard?.cardNumber ?? "4242 4242 4242 0000";
  const expiry = createdCard?.expiry ?? "12/29";
  const cvc = createdCard?.cvc ?? "•••";
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await readJson<{ payload: IssueCardPayload; card: Card }>(response);
  ...
  return (
    <>
      <PageHeader
        title="Issue Card"
        subtitle="Luis’s operating surface for creating voucher cards and handing beneficiaries a ready-to-use virtual number."
      />
                  ...
                  <div className="mt-6 flex items-end justify-between gap-4">
                    ...
                    <div className="flex gap-6 text-right">
                      ...
                      <div>
                        ...
                        <p className="mt-2 font-mono text-[15px] text-white">{cvc}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {createdCard ? (
                <div className="mt-5 rounded-[20px] border border-olive-300 bg-olive-50 p-4">
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-3">
                    <div>
                      <p className="text-[12px] text-ink-muted">PAN</p>
                      <p className="mt-1 font-mono text-[14px] text-ink">{createdCard.cardNumber}</p>
                    </div>
                    <div>
                      <p className="text-[12px] text-ink-muted">Expiry</p>
                      <p className="mt-1 font-mono text-[14px] text-ink">{createdCard.expiry}</p>
                    </div>
                    <div>
                      <p className="text-[12px] text-ink-muted">CVC</p>
                      <p className="mt-1 font-mono text-[14px] text-ink">{createdCard.cvc}</p>
                    </div>
                  </div>
                  <p className="mt-3 text-[13px] leading-6 text-ink-muted">
                    In the real Stripe flow, reveal uses an ephemeral key. For the hackathon build, this keeps the handoff visible and testable.
                  </p>
                </div>
              ) : null}
            </div>
